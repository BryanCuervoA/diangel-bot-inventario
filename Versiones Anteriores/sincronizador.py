...
print("🤖 Iniciando Robot Sincronizador Multilínea (Oro y Plata)...")

# ==========================================
# CONFIGURACIÓN GENERAL Y LLAVES
# ==========================================
...
def mapear_fotos_de_drive():
    print("🔍 Escaneando carpetas de fotos en Google Drive (saltando el límite de 100)...")
    ...
    print(f"📸 Se encontraron fotos para {len(diccionario_fotos)} referencias en Drive.")
...
try:
    ...
    print(f"⚠️ No se pudieron cargar las fotos de Drive: {e}. Se subirá el catálogo con fotos vacías.")


# ==========================================
# FASE 2: DESCARGAR EXCELES DE BELATRIZ
# ==========================================
opciones = Options()
...
try:
    ...
    driver.find_element(By.ID, 'Documento').send_keys('AQUI VA MI USUARIO')
    driver.find_element(By.ID, 'password').send_keys('AQUI VA MI CONTRASEÑA')
    ...
    print("✅ Excel de Oro descargado exitosamente.")

    print("🔄 Inyectando código JavaScript para cambiar a la línea Silver...")
    ...
    print("📥 Viajando a la biblioteca para descargar catálogo de Silver...")
    driver.get('https://belatrizcolombia.com/app/public/biblioteca')
    time.sleep(3)
    driver.find_element(By.ID, 'btnExportar').click()

    esperar_y_renombrar(archivo_plata)
    print("✅ Excel de Plata descargado exitosamente.")
...
# ==========================================
# FASE 3: TRANSFORMAR Y FUSIONAR DATOS (PANDAS)
# ==========================================
print("🧠 Fusionando catálogos y procesando datos...")
...
# 🔥 NUEVO CEREBRO: Función para extraer variantes exactas (Ajustada para línea Silver)
def extraer_variantes(referencia):
    ...
    # Identificar si tiene un prefijo de línea (ej. 'S-' para Silver)
    ...
    # Extraer la variante real si existe una posición después del código base
    if len(partes) > indice_variante:
        variante = partes[indice_variante].strip().upper()
    else:
        variante = ''
    ...
    # REGLA 1: Ignorar dijes de iniciales (1 sola letra)
    if len(variante) == 1:
        return referencia, ''

    # REGLA 2: Ignorar tallas (Empieza con T y sigue con números)
...
# 🔥 BONUS: Mapeo de fotos inteligente (Busca el código exacto primero, si no lo halla, busca la carpeta del código base)
...
# ==========================================
print("☁️ Subiendo catálogo unificado a Google Sheets...")
...
try:
    ...
    print("🎉 ¡Sincronización completada! Base de datos estructurada con variantes lista.")

except Exception as e:
    print(f"❌ Error al subir los datos a Google Sheets: {e}")